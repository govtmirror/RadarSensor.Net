/*$Header: /SAL/examples/Demos/Common/AgSal.cs 180 2014/04/21 01:30:18 -0700 lelanyue $*/
/****************************************************************************
 *
 * Copyright (c) Agilent Technologies 2008
 *
...
namespace AgSal
{
    public sealed class AgSalLib
    {
        ...
        private static extern SalError salSensorTest_Win32(IntPtr sensorHandle, int reserved, out string testResults, int resultsSize);

        [DllImport(AGSAL_DLL_NAME_X64, EntryPoint = "salSensorTest", CharSet = CharSet.Ansi)]
        private static extern SalError salSensorTest_X64(IntPtr sensorHandle, int reserved, out string testResults, int resultsSize);

        public static SalError salSensorTest(IntPtr sensorHandle, int reserved, out string testResults, int resultsSize)
        {
            if (IntPtr.Size == 8)
                return salSensorTest_X64(sensorHandle, reserved, out testResults, resultsSize);
            else
                return salSensorTest_Win32(sensorHandle, reserved, out testResults, resultsSize);
        }
        ...
        public struct LocateMeasParms3
        {
            ...
            UInt32 numMeasurements;        /**< Number of consecutive IQ measurements the sensor will execute when the sensor receives this measurement request.
	                                                If this is 0, then this number is changed to 1, and 1 IQ measurement is executed. */
            double measurePeriod;          /**< Amount of time in seconds between each consecutive IQ trigger times on the sensor. */
        }
        ...
        private static extern SalError salGetTimeseries_Win32(IntPtr smsHandle, Int32 measurementId, ref TimeseriesInfo2 info, float[] buffer, UInt32 numBytesInData);

        [DllImport(AGSAL_DLL_NAME_X64, EntryPoint = "salGetTimeseries", CharSet = CharSet.Ansi, SetLastError = true)]
        private static extern SalError salGetTimeseries_X64(IntPtr smsHandle, Int32 measurementId, ref TimeseriesInfo2 info, float[] buffer, UInt32 numBytesInData);

        public static SalError salGetTimeseries(IntPtr smsHandle, Int32 measurementId, ref TimeseriesInfo2 info, float[] buffer, UInt32 numBytesInData)
        {
            if (IntPtr.Size == 8)
                return salGetTimeseries_X64(smsHandle, measurementId, ref info, buffer, numBytesInData);
            else
                return salGetTimeseries_Win32(smsHandle, measurementId, ref info, buffer, numBytesInData);
        }
        ...
        public unsafe struct TimeDataParms
        {
            ...
            public UInt32 triggerTimeSecs;     /**< When trigger is absolute time, this is POSIX time, when trigger is relative time, this is delay from now */
